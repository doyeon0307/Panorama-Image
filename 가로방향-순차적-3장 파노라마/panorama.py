...
def find_correspondences(image1: np.ndarray, image2: np.ndarray,
    ...
    if matches:
        filtered_matches = []
        min_distance = height * 0.02  # 최소 거리
...
def stitch_images_with_precomputed(img1: np.ndarray, img2: np.ndarray, 
                                 corners1=None, corners2=None) -> np.ndarray:
    """개선된 이미지 스티칭 (전처리된 데이터 활용)
    Args:
        img1, img2: 입력 이미지들
        corners1, corners2: 미리 계산된 코너점들 (없으면 새로 계산)
    """
    # 1. 전처리 (corners가 제공되지 않은 경우에만)
    gray1 = np.mean(img1, axis=2).astype(np.float32)
    gray2 = np.mean(img2, axis=2).astype(np.float32)
...
def create_panorama(images: List[np.ndarray]) -> np.ndarray:
    ...
    # 중앙 이미지(index=1)와 오른쪽 이미지(index=2) 먼저 스티칭
    right_result = stitch_images(images[1], images[2])
    if right_result is None:
        print("오른쪽 이미지 스티칭 실패")
        return None
    
    # 그 다음 왼쪽 이미지(index=0)와 right_result 스티칭
    final_result = stitch_images(images[0], right_result)
    if final_result is None:
        print("왼쪽 이미지 스티칭 실패")
        return right_result  # 최소한 오른쪽 스티칭 결과는 반환
